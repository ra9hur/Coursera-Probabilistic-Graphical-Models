...
for i=1:numPeople
  ...
  if (i>2)
    genotypeVarChild = i;
    genotypeVarParentOne = factorList(1).var;
    genotypeVarParentTwo = factorList(2).var;
    factorList(i) = genotypeGivenParentsGenotypesFactor(numAlleles, genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo);
  endif
endfor

for i = 1:numPeople
	factorList(2*numPeople+i) = phenotypeGivenGenotypeFactor(alphaList,i,2*numPeople+i);
endfor
%}