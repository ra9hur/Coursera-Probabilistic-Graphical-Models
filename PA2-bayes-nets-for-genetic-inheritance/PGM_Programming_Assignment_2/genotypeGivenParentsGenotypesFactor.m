...
m = nchoosek(numAlleles,2)+numAlleles;
genotypeFactor.card = [m,m,m];

assign = IndexToAssignment([1:27],genotypeFactor.card);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

% Iterate through genotype that the parent could have gotten from its father
for i = 1:m
	ia = genotypesToAlleles(i,:);
% Iterate through genotype that the parent could have gotten from its mother
	for j = 1:m
