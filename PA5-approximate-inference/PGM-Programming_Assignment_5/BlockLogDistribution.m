...
% Extracting factor list corresponding to variables
factorList=G.var2factors{V(1)};    % Factors corresponding to variable 1=1 17 20
for i=2:length(V)
  factorList = unique([factorList,G.var2factors{V(i)}]);
endfor


fp = F(factorList(1));
for i=2:length(factorList)
%  factorList(i);                   % 1 , 17, 20
%  F(factorList(i));                % F(1) var=1; F(17) var=1,2;  F(20) var=1,2,5
  fp = FactorProduct(fp,F(factorList(i)));
endfor
...
for i = 1:d
  GetValueOfAssignment(fp,x(i,:),1:length(A));
...
For example, 
if V = [A, B], 
while the variable set is [A, B, C, D, E, F] with a given evidence set [2, 2, 1, 2, 1, 2], 

let F1 and F2 be the factors that contain A and/or B. 
temp = G.var2factors{V}

What you are looking to find is 
F1(A=1, B=1 | C=1, D = 2, E = 1, F = 2)
F1(A=2, B=2|C=1, D = 2, E = 1, F = 2)
F2(A=1, B=1 | C=1, D = 2, E = 1, F = 2)
F2(A=2, B=2|C=1, D = 2, E = 1, F = 2)

What you need now, is 
LogBS(1) = log(F1(A=1, B=1 | C=1, D = 2, E = 1, F = 2)) + log(F2(A=1, B=1|C=1, D = 2, E = 1, F = 2))
LogBS(2) = log(F1(A=2, B=2 | C=1, D = 2, E = 1, F = 2)) + log(F2(A=2, B=2|C=1, D = 2, E = 1, F = 2))
%}