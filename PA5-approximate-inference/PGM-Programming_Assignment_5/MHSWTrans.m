...
% p_acceptance = min(1, pi_ratio*r_ratio*QY_ratio) (each ratio has exp on numerator and denominator)


log_pi_accept = LogProbOfJointAssignment(F, A_prop);  % uses proposed A
log_pi = LogProbOfJointAssignment(F, A);              % uses current A

p_acceptance = min(1,exp(log_pi_accept-log_pi+log_QY_ratio+LogR(old_value)-LogR(new_value)));
