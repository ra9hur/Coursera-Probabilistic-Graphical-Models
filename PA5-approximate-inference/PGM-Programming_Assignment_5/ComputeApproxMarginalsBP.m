...
function [M resid] = ComputeApproxMarginalsBP(F,E)

    % returning approximate marginals.
    
    clusterGraph = CreateClusterGraph(F,E);
    
    [P MESSAGES resid] = ClusterGraphCalibrate(clusterGraph);
    ...
    for i = 1:length(N),
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	      ...
	      for j = 1:length(clusterGraph.clusterList)
		      inter = intersect(vars(i),clusterGraph.clusterList(j).var);  % Variables that i,j choose to talk about
		      if length(inter)==1
			      V = setdiff(clusterGraph.clusterList(j).var,vars(i));      % All other variables other than in sepset
			      M(i) = FactorMarginalization(P.clusterList(j),V);
			      break;
		      endif
	      endfor        
